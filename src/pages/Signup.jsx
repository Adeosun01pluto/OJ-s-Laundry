  ...
  try {
    ...
    if (!userDoc.exists()) {
      await setDoc(userDocRef, {
        email: user.email,
        name: '', // You can prompt the user to provide this later
        photoUrl: '',
        bio: '',
...
const signUpWithProvider = async () => {
  ...
  try {
    ...
    if (!userDoc.exists()) {
      // If user doesn't exist, create a new user document in Firestore
      await setDoc(userDocRef, {
        email: user.email,
    ...
    // Navigate to the desired route after signup or login
    navigate('/');
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          <div className='flex items-center w-full justify-center gap-2 pt-3'>
            <p>Already have an account? </p> <Link to="/login" className='font-semibold text-[#275C9E]'>Login</Link>
          </div>
          ...
          <div className="mt-6">
            ...
            <div className="mt-3 grid grid-cols-1 gap-3">
              <div>
                ...
                >
                  <span className="sr-only">Sign un with Google</span>
                  <svg
